...
> **Fork notice.** This is a fork of [`edgeandnode/gateway`](https://github.com/edgeandnode/gateway)
> (MIT, v27.6.0) maintained by the Lodestar team. Upstream is the canonical implementation and the
> source of truth for the query-serving and TAP-payment logic; we track it and diverge only where an
> independent operator's needs differ. See [`LICENSE`](LICENSE) for the original copyright and
> [`PLAN.md`](PLAN.md) for the standing-up roadmap and corrected research notes.
...
Linux (and the provided `Dockerfile`) build out of the box — `rdkafka` statically builds its
vendored `librdkafka`. Just `cargo build --release`.

On **macOS** the vendored `librdkafka` C++ wrapper fails against modern libc++, so link the
Homebrew copy via the `dynamic-kafka` feature:
...
A gitignored `.cargo/config.toml` (see the example in this repo's history / `PLAN.md`) sets the
`PKG_CONFIG_PATH`/`OPENSSL_ROOT_DIR` needed for pkg-config to find the kegs.
...
given as the first argument to the lodestar-gateway executable.
e.g. `lodestar-gateway path/to/config.json`. The structure of the configuration file is defined in
[config.rs](src/config.rs) (`lodestar_gateway::config::Config`).

Log filtering is set using the `RUST_LOG` environment variable. For example, if you would like to
set the default log level to `info`, but want to set the log level for the `lodestar_gateway` module to
`debug`, you would use `RUST_LOG="info,lodestar_gateway=debug"`. More details on environment variable
